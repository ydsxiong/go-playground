...
func (s *loggingService) LoadCargo(id cargo.TrackingID) (c CargoView, err error) {
	defer func(begin time.Time) {
		_ = s.logger.Log(
			"method", "load",
			"tracking_id", id,
...
func (s *loggingService) Cargos() []CargoView {
	defer func(begin time.Time) {
		_ = s.logger.Log(
			"method", "list_cargos",
			"took", time.Since(begin),
...
func (s *loggingService) Locations() []LocationView {
	defer func(begin time.Time) {
		_ = s.logger.Log(
